...
const RestaurantApp: React.FC<Props> = ({ orders, setOrders }) => {
  ...
  const restaurantOrders = orders.filter(o => o.restaurantId === 'r1');
      ...
      <div className="p-8 bg-white border-b rounded-b-[40px] shadow-sm">
        ...
        <div className="flex gap-4 mt-6">
           <div className="flex-1 bg-slate-50 p-4 rounded-3xl border border-slate-100">
              ...
              <p className="text-xl font-black text-slate-900">{restaurantOrders.length}</p>
           </div>
        ...
        {activeTab === 'orders' && (
          <div className="space-y-4 animate-in slide-in-from-bottom duration-500">
             {restaurantOrders.map(order => (
               <div key={order.id} className="bg-white rounded-[32px] p-6 shadow-sm border">
                  <div className="flex justify-between items-start mb-4">
                     <div>
                        <h4 className="font-black text-slate-900 text-sm">Order {order.id}</h4>
             ...
             {restaurantOrders.length === 0 && (
                 <div className="text-center py-20 opacity-20 font-black uppercase text-xs">Waiting for orders</div>
             )}
          </div>
        )}
      </div>
